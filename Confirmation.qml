...
Page {
    anchors.fill: parent
    title: qsTr("تایید خروج از پارکینگ")

    Text {
        id: text1
        text: qsTr("جهت تایید خروج، یکی از فضاهای اشغال‌شده را انتخاب کنید.")
        font.family: "Tahoma"
        font.pointSize: 15
        width: parent.width
    ...
    Image {
        ...
        function changeCarState(c) {
            if (c.source == "qrc:/Images/car-red.png") {
                for (var i = 0; i < map.children.length; i++) {
    ...
    MessageDialog {
        id: messageDialog
        title: "خروج از پارکینگ"
        text: "خروج از پارکینگ با موفقیت ثبت شد!"
        icon: "Information"
    }
}
